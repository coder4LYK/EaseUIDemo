    ...
    if (![self isEmpty]) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [weakself hideHud];
                if (!error) {
                    UIAlertView *aler = [[UIAlertView alloc] initWithTitle:@"注册成功" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
                    [aler show];
...
//                        [[ChatDemoHelper shareHelper] asyncPushOptions];
                        ...
                        [self.navigationController pushViewController:[[ConversationListViewController alloc] init] animated:YES];
                        //保存最近一次登录用户名
